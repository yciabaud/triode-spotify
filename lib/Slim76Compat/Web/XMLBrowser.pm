package Slim76Compat::Web::XMLBrowser;

# $Id: XMLBrowser.pm 33307 2011-09-05 13:42:32Z mherger $
...
sub handleFeed {
	...
	if ( my $levels = scalar @index ) {
		...
		for my $i ( @index ) {
			...
			{
				($feedAction, $feedActions) = Slim76Compat::Control::XMLBrowser::findAction($superFeed, $subFeed, $1);
			}
			...
			if ( defined $subFeed->{'url'} && !$subFeed->{'fetched'} ) {
				...
				my ($feedAction, $feedActions) = Slim76Compat::Control::XMLBrowser::findAction( $superFeed, $subFeed, 
...
sub _makeWebLink {
	...
	my ($feedAction, $feedActions) = Slim76Compat::Control::XMLBrowser::findAction($feed, $item, $action);
	if ($feedAction) {
...
sub _makePlayLink {
	...
	my ($feedAction, $feedActions) = Slim76Compat::Control::XMLBrowser::findAction({actions => $feedActions}, $item, $action);
	if ($feedAction) {
